...
int main (int argc, char** argv)
{
	...
	if (mpi_rank != 0)
	{
		printf("Sending for rank = %d\n", mpi_rank);
		MPI_Isend(&c[0], d.nx*d.ny, MPI_INT, 0, ping, p.comm, &request);
	...
	if (mpi_rank == 0)
	{
		...
		for (int proc = 1; proc < mpi_size; proc++)
		{
			...
			printf("receiving for rank = %d\n", proc);
			MPI_Irecv(&c[0], d1.nx * d1.ny, MPI_INT, proc, ping, p1.comm, &request);
			MPI_Wait(&request, &status);
