function [out_opd] = SPACL_kmeans_dim_entropy_analytic_gauge(X,pi,K,N_anneal,out_init,reg_param,X_valid,pi_valid,N_neurons,flag_AUC,flag_parallel,RandomSeed)
...
out_opd.RandomSeed=rng;
end

function [out]=SPACL_Replica(in)
    dim_red=in.reg_param(1);
    X=in.X;
    gamma=in.gamma;
    W=in.W;
    X_valid=in.X_valid;
    ...
    ind_dim=randperm(d);
   W=[eye(d)];
   W=W(:,ind_dim(1:dim_red));
    while and(delta_L>eps,i<=MaxIter)
      %[L_3]=SPACL_dim_entropy_L(X,pi,C,Lambda,gamma,T,d,m,  reg_param, eps_C,W);
      % L_3
       [C]=SPACL_EvaluateCRegularize_gauge(X,gamma,W,K,d,T);
      %[L_3]=SPACL_gauge_L(X,pi,C,Lambda,gamma,T,d,m,  reg_param,W)

      [gamma]=SPACL_EvaluateGamma(X,pi,W*C,Lambda,T,K,m,d,reg_param);
      %[L_3]=SPACL_gauge_L(X,pi,C,Lambda,gamma,T,d,m,  reg_param,W)
      ...
      % L_3
       ...
       [U,S,Z]=svds(X*gamma'*C',dim_red);
       [W]=U*Z';
       %[W]=SPACL_dim_entropy_EvaluateWRegularize(X,gamma,C,d,T,W,eps_C);
       [L_3]=SPACL_gauge_L(X,pi,C,Lambda,gamma,T,dim_red,m,  reg_param,W);
...
%    [gamma_valid,LLL2]=EvaluateGammaParallel(X_valid,C,T_valid,d,K,[],xt2_valid);
    ...
    if L_pred_Markov_train1< L_pred_Markov_train2
        [gamma_valid]=EvaluateGammaParallel_Lukas_vect(X_valid,W*C,T_valid,d,K,[],xt2_valid);
    else
        [gamma_valid]=SPACL_EvaluateGamma_valid(X_valid,W*C,T_valid,K);
    end
    gamma_valid=real(gamma_valid);
    [Lambda_valid]=SPACL_EvaluateLambdaRegularize(pi_valid,gamma_valid,m,K);
