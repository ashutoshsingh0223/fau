function [out_opd] = ConvMeansParallelRegularize_Classify_GO_v2(X,pi,K,N_anneal,gamma_init,reg_param,X_valid,pi_valid,N_neurons,GO)
...
for kkk=1:numel(in)
    [out{kkk}]=KHullsAlgorithmReplica_Classify_GO_v2(in{kkk});
end
for kkk=1:numel(in)
    n=in{kkk}.n;
    e=in{kkk}.e;
...
function [out]=KHullsAlgorithmReplica_Classify_GO_v2(in)
    X=in.X;
    ...
    while and(delta_L>eps,i<=MaxIter)
        [C,L1,norm_C]=EvaluateCRegularize_GO_v2(X,gamma,T,d,K,xt2,H2,reg_param);
