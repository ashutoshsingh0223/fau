function [out] = kmeans_bayes_dim_entropy(X,pi,K,N_anneal,reg_param)
...
%        pis=Lambda*gamma2;figure(13);clf;plot(Xx(1,round(pis(1,:))==1),Xx(2,round(pis(1,:))==1),'r.');hold on;plot(Xx(1,round(pis(1,:))==0),Xx(2,round(pis(1,:))==0),'bx');plot(C(1,:),C(2,:),'ko','LineWidth',3,'MarkerSize',10)
    ...
    out.time=toc;
    out.L=L_3;%SPACL_dim_entropy_L(X_W,pi,C,Lambda,gamma,T,d,m, 0, 0,W,K,eps_Creg);
    ...
    %LogLik = -out.L*T*d;%sum(sum(pi.*(log(max(Lambda*gamma_valid,1e-12)))));
    out.N_params=dd+dd*K;%d*(K+1);%length(W)*(K+1);%length(find(W>1e-5))*(K+1);
    out.BIC=sum(W.*log(max(W,1e-12)));
    end
