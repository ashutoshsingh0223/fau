function [out_opd] = SPACL_plus_dim_entropy(X,pi,K,N_anneal,out_init,reg_param,X_valid,pi_valid,N_neurons,flag_AUC)
...
function [out]=SPACL_Replica(in)
    ...
    while and(delta_L>eps,i<=MaxIter)
        ...
        %pis=Lambda*gamma1;figure(13);clf;plot(Xx(1,round(pis(1,:))==1),Xx(2,round(pis(1,:))==1),'r.');hold on;plot(Xx(1,round(pis(1,:))==0),Xx(2,round(pis(1,:))==0),'bx');plot(C(1,:),C(2,:),'ko','LineWidth',3,'MarkerSize',10)
...
%         end
 ...
 %[gamma]=SPACL_dim_entropy_EvaluateGamma(X_W,pi,C,Lambda,T,K,m,d,0,gamma,W);
 %[gamma1,LLL2]=EvaluateGammaParallel_Lukas_vect(diag(sqrt(W))*X,C,T,d,K,[],xt2_valid);
 %   [L_pred_Markov_train1] = AUC_of_Prediction(gamma1,P,pi,0,flag_AUC);
 %   L_pred_Markov_train1=L_pred_Markov_train1/(T*m);
    
%    [gamma2]=SPACL_EvaluateGamma_valid(diag(sqrt(W))*X,C,T,K);
...
%        [gamma_valid,LLL2]=EvaluateGammaParallel_Lukas_vect(diag(sqrt(W))*X_valid,C,T_valid,d,K,[],xt2_valid);
%    else
%        [gamma_valid]=SPACL_EvaluateGamma_valid(diag(sqrt(W))*X_valid,C,T_valid,K);
%    end
