function [out_opd] = SPACL_kmeans_dim_entropy_analytic_v2(X,pi,K,N_anneal,out_init,reg_param,X_valid,pi_valid,N_neurons,flag_AUC)
...
%        pis=Lambda*gamma2;figure(13);clf;plot(Xx(1,round(pis(1,:))==1),Xx(2,round(pis(1,:))==1),'r.');hold on;plot(Xx(1,round(pis(1,:))==0),Xx(2,round(pis(1,:))==0),'bx');plot(C(1,:),C(2,:),'ko','LineWidth',3,'MarkerSize',10)
    ...
    [gamma1,LLL2]=EvaluateGammaParallel_Lukas_vect(diag(sqrt(W))*X,C_W,T,d,K,[],xt2_valid);
...
%    [gamma_valid,LLL2]=EvaluateGammaParallel(X_valid,C,T_valid,d,K,[],xt2_valid);
    ...
    if L_pred_Markov_train1< L_pred_Markov_train2
        [gamma_valid,LLL2]=EvaluateGammaParallel_Lukas_vect(diag(sqrt(W))*X_valid,C_W,T_valid,d,K,[],xt2_valid);
    else
        [gamma_valid]=SPACL_EvaluateGamma_valid(diag(sqrt(W))*X_valid,C_W,T_valid,K);
    end
