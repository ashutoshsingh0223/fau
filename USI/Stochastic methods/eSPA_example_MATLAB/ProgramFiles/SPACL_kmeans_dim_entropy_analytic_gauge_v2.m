function [out_opd] = SPACL_kmeans_dim_entropy_analytic_gauge_v2(X,pi,K,N_anneal,out_init,reg_param,X_valid,pi_valid,N_neurons,flag_AUC,RandomSeed)
...
function [out]=SPACL_Replica(in)
   ...
   W=W(:,ind_dim(1:dim_red));
    while and(delta_L>eps,i<=MaxIter)
      ...
      % L_3
       [C]=SPACL_EvaluateCRegularize_gauge(W'*X,gamma,K,dim_red,T);
      [gamma]=SPACL_EvaluateGamma(W'*X,pi,C,Lambda,T,K,m,dim_red,reg_param);
      ...
      % L_3
       ...
       [L_3]=SPACL_gauge_L_v2(X,pi,C,Lambda,gamma,T,d,m,  reg_param,W);
...
%        pis=Lambda*gamma2;figure(13);clf;plot(Xx(1,round(pis(1,:))==1),Xx(2,round(pis(1,:))==1),'r.');hold on;plot(Xx(1,round(pis(1,:))==0),Xx(2,round(pis(1,:))==0),'bx');plot(C(1,:),C(2,:),'ko','LineWidth',3,'MarkerSize',10)
    ...
    [gamma1]=EvaluateGammaParallel_Lukas_vect(W'*X,C,T,dim_red,K,[],xt2_valid);
...
%    [gamma_valid,LLL2]=EvaluateGammaParallel(X_valid,C,T_valid,d,K,[],xt2_valid);
    ...
    if L_pred_Markov_train1< L_pred_Markov_train2
        [gamma_valid]=EvaluateGammaParallel_Lukas_vect(X_valid,W*C,T_valid,dim_red,K,[],xt2_valid);
    else
