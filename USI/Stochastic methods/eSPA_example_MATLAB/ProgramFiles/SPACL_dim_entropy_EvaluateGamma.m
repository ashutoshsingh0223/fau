function [gamma]=SPACL_dim_entropy_EvaluateGamma(X,pi,C,Lambda,T,K,m,d,reg_param,gamma,W)
%UNTITLED Summary of this function goes here
%   Detailed explanation goes here
options=optimset('GradObj','on','Algorithm','interior-point','MaxIter',200,'Display','off','TolFun',1e-13,'TolCon',1e-13,'TolX',1e-13,...
    'TolConSQP',1e-13,'TolGradCon',1e-13,'TolPCG',1e-13);
A=-eye(K);b=zeros(K,1);
Aeq=ones(1,K);beq=1;
eps1=-reg_param/T/m;eps2=1/T/d;
%C=diag(sqrt(W))*C;
%X=diag(sqrt(W))*X;
CTC=C'*C;mm=min(abs(eps1),abs(eps2));
...
for t=1:T
    ...
    [fun]=LogLik_SPACL_gamma(xxx0,X(:,t),pi(:,t),Lambda,eps1,eps2,C,CTC);
    if fun<fff
        %t
        gamma(:,t)=xxx0;
    end
