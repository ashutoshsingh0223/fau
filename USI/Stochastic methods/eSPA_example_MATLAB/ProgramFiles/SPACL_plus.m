function [out_opd] = SPACL_plus(X,pi,K,N_anneal,out_init,reg_param,X_valid,pi_valid,N_neurons,flag_AUC)
...
function [out]=SPACL_Replica(in)
    ...
    while and(delta_L>eps,i<=MaxIter)
       %L_3
        [gamma]=SPACL_plus_EvaluateGamma(X,pi,C,Lambda,T,K,m,d,reg_param,gamma);
       %[L_3,Lfin,norm_C]=SPACL_plus_L(X,pi,C,Lambda,gamma,T,d,m,K,  reg_param, eps_C);
       %L_3
