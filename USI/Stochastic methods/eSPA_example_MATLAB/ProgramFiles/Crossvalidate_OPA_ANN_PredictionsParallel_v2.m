function [ err,net] = Crossvalidate_OPA_ANN_PredictionsParallel_v2(in)
...
net = patternnet(N_neurons);net.trainParam.showWindow = 0;
net = train(net,squeeze(pi_train(:,1,:)),squeeze(pi_train(:,3,:)),'useParallel','no');    
err=Estimate_OPA_ANN_PredictionError(net,C,pi_valid,x_valid);
...
function err=Estimate_OPA_ANN_PredictionError(net,C,pi,x)
TTT=size(pi,3);
d=size(C,1);
%P=C*P^N_steps;
