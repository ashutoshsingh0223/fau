...
reg_param_W=[8e-3];
reg_param_CL=[1e-10 1e-9 1e-8 5e-8 1e-7 5e-7 1e-6 5e-6 8e-6 1e-5 2e-5 5e-5 6e-5 7e-5 1e-4 2e-4 3e-4 5e-4 8e-4  1e-3 2e-3 5e-3 1e-2];
k=1;
...
VisualizeVoronoi(out_eSPA.C(1:2,:),out_eSPA.P_valid(1,:))
[~,ind1]=find(pi(1,:)==1);[~,ind2]=find(pi(2,:)==1);
hold on;plot(X(1,ind1),X(2,ind1),'ro','LineWidth',2);hold on;plot(X(1,ind2),X(2,ind2),'kx','LineWidth',2);
caxis([0 1])
