...
def visualize_rewards_vs_episodes(q_learning_rewards, sarsa_rewards):
    x_axis = np.arange(0, len(q_learning_rewards))

    q_learning_rewards, sarsa_rewards = np.array(q_learning_rewards, dtype=np.float32), np.array(sarsa_rewards, dtype=np.float32)

    print(len(sarsa_rewards), len(q_learning_rewards))
    plt.figure(figsize=(7, 7))
    plt.plot(x_axis[::500], q_learning_rewards[::500], label='Q-Learning', ls='-')
    # plt.plot(sarsa_rewards, color='y', label='SARSA')
