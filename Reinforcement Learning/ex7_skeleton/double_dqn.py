...
class DoubleDQN():
    ...
    def compute_msbe_loss(self):
        ...
        with torch.no_grad():
            # Taking actions that have maximum q value for next state as predicted by main net
            next_q_values = self.dqn_target_net(next_obs).cpu()
            max_actions = np.argmax(self.dqn_net(next_obs).cpu(), axis=1)
...
if __name__ == '__main__':
    ...
    # dqn = DoubleDQN(env)
    # dqn.learn(10000)
    # visualize_agent(env, dqn, num_timesteps=500)
