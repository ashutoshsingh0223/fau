...
def inliers_epipolar_constraint(p1: t_points, p2: t_points, F:t_homography, distance_threshold: float) -> np.array:
    ...
    p1_homo = np.ones((p1.shape[0], p1.shape[1] + 1))
    p1_homo[:, :-1] = p1

    p2_homo = np.ones((p2.shape[0], p2.shape[1] + 1))
    p2_homo[:, :-1] = p2
    proj = np.matmul(F, p1_homo.T).T
    # Step 2 - normalize the projected points (2 lines)
    norm_proj = (proj / proj[:, -1].reshape(len(proj), 1))
    # Step 3 - compute the distance (1 line)
    # D = []
    # for i in range(proj.shape[0]):
    #     D.append(np.abs(np.dot(p2_homo[i].T, norm_proj[i])))
    # D = np.array(D)

    D = np.matmul(p2_homo, norm_proj.T)
    # print(distance_threshold)
    # Step 4 - check if the distance is smaller than t and return (2 lines)
    # print(D)
    inliers = np.where(D < distance_threshold)[0]
    return inliers
...
def compute_fundamental_matrix(points1: t_points, points2: t_points) -> np.array:
    ...
    for i in range(8):
        p_x, p_y = points1[i]
        q_x, q_y = points2[i]
        A[i] = np.array([p_x*q_x, p_x*q_y, p_x, p_y*q_x, p_y*q_y, p_y, q_x, q_y, 1], dtype=np.float64)
    ...
    print(S)
    print(np.diag(S))
    fundamental_matrix = np.matmul(np.matmul(U, np.diag(S)), V).T
...
def compute_essential_matrix(fundamental_matrix: t_homography, camera_parameters: t_camera_parameters) -> t_homography:
    ...
    essential_matrix = np.matmul(np.matmul(camera_parameters.T, fundamental_matrix), camera_parameters)
    essential_matrix = essential_matrix / essential_matrix[-1, -1]
...
def decompose(E: t_homography) -> Tuple[np.array, np.array, np.array, np.array]:
    ...
    u3 = U[-1]
    s = np.divide(u3, np.linalg.norm(u3))
    t1 = s
    t2 = np.multiply(-1, s)
